...
BEGIN
  ...
  BEGIN
    ...
    ---------------INSERT------------------------
   CREATE OR REPLACE PROCEDURE INS_DYNAMIC(
  P_TABLE   VARCHAR2,
  P_COLUMNS VARCHAR2,
  VAL1      VARCHAR2,
  VAL2      VARCHAR2,
...
AS
  V_SQL VARCHAR2(1000);
BEGIN
  V_SQL := 'INSERT INTO ' || P_TABLE || ' (' || P_COLUMNS || ') VALUES (:1, :2, :3)';
  EXECUTE IMMEDIATE V_SQL USING VAL1, VAL2, VAL3;
  DBMS_OUTPUT.PUT_LINE('INSERTED: ' || V_SQL);
END;

BEGIN
  INS_DYNAMIC('DEPT', 'DEPTNO, DNAME, LOC', '73', 'RESEARCH', 'HUSTON');
END;

------------DELETE-------------------
CREATE OR REPLACE PROCEDURE DEL_DYNAMIC(
  P_TABLE VARCHAR2,
...
BEGIN
  DEL_DYNAMIC('DEPT', 'DEPTNO = 73');
END;


    
    SELECT * FROM DEPT WHERE DEPTNO=93;
